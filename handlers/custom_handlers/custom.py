...
def custom(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, Variables.custom_pr, message.chat.id)


@bot.message_handler(state=Variables.custom_pr)
def get_custom(message: Message) -> None:
    ...
    for elem in json_dict_max['data'][(int(Variables.custom_pr.split()[0])):(int(Variables.custom_pr.split()[1]))]:
        price = elem['price']
