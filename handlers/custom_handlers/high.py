...
def high(message: Message) -> None:
    bot.set_state(message.from_user.id, Variables.max_pr, message.chat.id)
    bot.send_message(message.chat.id, 'Введите требуемое количество самых дорогих вариантов:')


@bot.message_handler(state=Variables.max_pr)
def get_high(message: Message) -> None:
    ...
    for elem in json_dict_max['data'][(len(json_dict_max['data']) - int(Variables.max_pr)):len(json_dict_max['data'])]:
        price = elem['price']
